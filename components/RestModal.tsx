      ...
      <div className="w-[420px] bg-skyrim-paper p-6 rounded border border-skyrim-border">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 mt-2">
            <button onClick={() => setOption('camp')} className={`px-3 py-2 rounded ${option==='camp'?'bg-skyrim-gold/10 border-skyrim-gold':'bg-black/40 border-skyrim-border'}`}>Camp (50% restore)</button>
            <button onClick={() => setOption('bed')} className={`px-3 py-2 rounded ${option==='bed'?'bg-skyrim-gold/10 border-skyrim-gold':'bg-black/40 border-skyrim-border'}`} disabled={!hasBedroll}>Bed (90% restore)</button>
          </div>
        </div>

        <div className="mb-4">
          <label className="text-xs text-gray-400">Hours to rest</label>
          <input type="range" min={1} max={24} value={hours} onChange={e => setHours(Number(e.target.value))} />
          <div className="text-sm text-gray-300 mt-1">{hours} hour{hours!==1?'s':''} ({hours*60} minutes)</div>
        </div>
